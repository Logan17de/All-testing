...
- **Branch:** `zet-harness-v1` on GitHub holds all local work; the latest slice is 9.3 (run lineage).
...
- **Working on now — Phase 9:** 9.1 (recorded replay), 9.2 (forking a run from any point in
  another run's history) and 9.3 (that lineage in the run view, inspector and run list) are
  done, see `PHASE-9.md`; **9.4, refusing to resume a run against an edited graph**, is next.
...
Phase 9  Replay + memory + triggers        🚧 IN PROGRESS
           ...
           ├─ 9.4 graph/run identity checks before resuming                     ▶ CURRENT
...
| **9 — Replay/Fork + Memory + Triggers + External clients** | recorded replay, checkpoint forks, lightweight memory, cron/webhook/API triggers, Copycat/client bridge | 🚧 In progress — 9.1 replay, 9.2 fork, 9.3 lineage done; 9.4 identity next (see `PHASE-9.md`) |
