...
- **Branch:** `zet-harness-v1` on GitHub holds all local work; the latest slice is 9.4 (plan identity checks).
...
- **Working on now — Phase 9:** 9.1 (recorded replay), 9.2 (forking a run), 9.3 (lineage in the
  run view and inspector) and 9.4 (refusing to resume a run whose plan or plugin code changed)
  are done, see `PHASE-9.md`; **9.5-9.8, project memory**, are next.
...
| **9 — Replay/Fork + Memory + Triggers + External clients** | recorded replay, checkpoint forks, lightweight memory, cron/webhook/API triggers, Copycat/client bridge | 🚧 In progress — 9.1-9.4 replay, fork, lineage, identity done; memory next (see `PHASE-9.md`) |
