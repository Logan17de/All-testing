...
- **Branch:** `zet-harness-v1` on GitHub holds all local work; the latest slice is 9.6 (memory in an agent's context).
...
- **Working on now — Phase 9:** 9.1 (recorded replay), 9.2 (forking a run), 9.3 (lineage),
  9.4 (refusing to resume changed work), 9.5 (what a project remembers) and 9.6 (offering that
  memory to an agent within its context budget) are done, see `PHASE-9.md`; **9.7, summarizing
  a conversation only when it no longer fits**, is next.
...
| **9 — Replay/Fork + Memory + Triggers + External clients** | recorded replay, checkpoint forks, lightweight memory, cron/webhook/API triggers, Copycat/client bridge | 🚧 In progress — 9.1-9.6 replay, fork, lineage, identity, memory, recall done; summarization next (see `PHASE-9.md`) |
