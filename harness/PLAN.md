...
### You are here (updated 2026-09-16)

- **Branch:** `zet-harness-v1` on GitHub holds all local work; Phase 9 is complete; the latest slice is 9.12 (the client bridge).
- **Phase 8, before this:** structured bounded loops, subgraphs, projects, conversations with
...
- **Just finished — Phase 9:** all twelve items are done, see `PHASE-9.md`: recorded replay,
  forking a run from any point in another's history, lineage in the inspector, refusing to
  resume work whose plan or plugin code changed, project memory with recall inside the
  agent's context budget, summarizing a conversation only when it no longer fits, memory
...
- **Working on now — Phase 11:** 11.3 adds `npm run backup`, which copies the database and
  ...
  11.2 adds `npm run setup` and `harness.config.json`, read under a settled order:
  environment, then file, then default.
- **Next:** 11.5 (graph versioning and diff), and the optional items 11.4 and 11.6-11.9.
- **Still open elsewhere:** 6.12 (needs a real local model endpoint), 10.10 (WASI plugins),
  11.4-11.9 (packaging and optional scale-out).
...
Phase 4  Runtime daemon + SQLite           ✅ COMPLETE
...
Phase 5  Effects + permissions + humans    ✅ COMPLETE
           ...
           ├─ 5.6 compile-time graph/node capability enforcement                ✅
...
Phase 8  Loops + projects + agent mode     ✅ COMPLETE
           ...
           ├─ 8.8 sortable IDs and epoch-ms timestamps everywhere              ✅
           ├─ 8.9 deterministic next runnable todo                             ✅
           ├─ 8.10 context builder with hard byte/token budgets                ✅
           ├─ 8.11 model-visible goal/todo actions, applied once               ✅
           ├─ 8.12 bounded model→tool→model agent loop on the structured loop   ✅
           ├─ 8.13 blocked state and goal-completion logic                     ✅
           ├─ 8.14 projects, chat, goals and todos UI                          ✅
           ├─ 8.15 multi-step coding integration test (scripted provider)      ✅
           ├─ 8.16 golden trace of a complete deterministic goal run           ✅
...
Phase 11 Packaging + optional scale-out    🚧 3/9
```
...
| **11 — Packaging + Optional scale-out** | Windows setup, config wizard, backup/import/export, optional desktop shell, optional Postgres/remote workers | 🚧 11.1-11.3 done; 11.4–11.9 pending |
...
Phase 4.21 adds a reproducible lightweight baseline command (`npm run baseline`) without adding production dependencies or hard performance thresholds. Fresh runtime child processes measure startup through module load, SQLite open/migrations, and HTTP readiness, then report idle RSS; the same command records the direct runtime dependency count from `apps/runtime/package.json`. A deterministic 64-op chain measures the real Graph JSON validation/normalization/canonicalization/lowering/compiler-identity path and the real `PlainDagRun` scheduler path, while SQLite timing exercises the serialized `SqliteDatabase.commit()` `BEGIN IMMEDIATE` path against a temporary file-backed WAL database with a prepared insert. Timing benchmarks use short warmup/sample windows and runtime startup/RSS report sample count, median, p95, minimum, and maximum. The command has been exercised successfully on hosted Ubuntu and Windows runners, whose materially different startup timings demonstrate why 4.22 must compare recorded baselines without brittle universal machine thresholds.

Phase 4.22 closes the durability phase with a deliberately coarse CI regression guard rather than a portable performance claim. `npm run baseline:ci` runs independently on `ubuntu-latest` and `windows-latest`; the benchmark writes runtime statistics directly to `tmp/baseline/runtime.json`, Vitest 4 writes benchmark medians to `tmp/baseline/bench.json`, and the checker writes `tmp/baseline/check.json` before returning success or failure. Each OS compares only with its own checked-in reference using broad `max(reference × multiplier, reference + additive slack)` ceilings for startup, idle RSS, compiler, scheduler, and file-backed WAL SQLite commit medians, while direct runtime dependency identity is checked exactly. Faster results always pass, cross-OS comparisons are forbidden, and every matrix job uploads the three JSON reports even when the guard fails. This makes large regressions review-visible without letting hosted-runner noise turn CI into a benchmark contest. Phase 4 is complete; Phase 5 now owns effect/idempotency/recovery enforcement and permission boundaries.
