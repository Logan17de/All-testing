...
- **Branch:** `zet-harness-v1` on GitHub holds all local work; the latest slice is 9.7 (summarizing only when needed).
...
- **Working on now — Phase 9:** 9.1-9.7 are done, see `PHASE-9.md`: recorded replay, forking a
  ...
  **9.8, full-text search, only if that retrieval proves insufficient**, is next.
...
| **9 — Replay/Fork + Memory + Triggers + External clients** | recorded replay, checkpoint forks, lightweight memory, cron/webhook/API triggers, Copycat/client bridge | 🚧 In progress — 9.1-9.7 done through summarization; triggers and clients next (see `PHASE-9.md`) |
