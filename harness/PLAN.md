...
- **Branch:** `zet-harness-v1` on GitHub holds all local work; the latest slice is 9.5 (project memory).
...
- **Working on now — Phase 9:** 9.1 (recorded replay), 9.2 (forking a run), 9.3 (lineage),
  9.4 (refusing to resume changed work) and 9.5 (what a project remembers) are done, see
  `PHASE-9.md`; **9.6, offering that memory to an agent within a context budget**, is next.
...
Phase 9  Replay + memory + triggers        🚧 IN PROGRESS
           ...
           ├─ 9.6 recent/pinned retrieval + context-budget accounting           ▶ CURRENT
...
| **9 — Replay/Fork + Memory + Triggers + External clients** | recorded replay, checkpoint forks, lightweight memory, cron/webhook/API triggers, Copycat/client bridge | 🚧 In progress — 9.1-9.5 replay, fork, lineage, identity, memory done; retrieval next (see `PHASE-9.md`) |
