...
- Blob bytes are published atomically and verified on reuse; `commitDurableNodeCompletion(...)` atomically wires output refs, completed attempt state, and its terminal durable event using the existing attempt/event schemas, so Phase 4.14 requires no new migration. Phase 4.15 now gates scheduler downstream release on successful completion durability; restart frontier reconstruction remains Phase 4.16.
